...
#Создаем окно с выше указынными знаечениями и даем название игры
...
class Wall:
    #Эта функция инициализирует уровень и открывет нам нужный файл, а так же все координаты стен закидывает в лист
    def __init__(self, level):
        self.level = level
        self.body = []
        f = open(f"levels\level{self.level}.txt", "r")
...
#Эта функция отвечает за отоброжение текущего уровня и счета
...
def main():
    ...
    #эти переменные будут отвечать за то как будет двигаться змея
    dx = 0
    dy = 0
    ...
    while status:
        ...
        #Ранее мы дали координаты как будет двигаться змея и сейчас мы даем координаты для головы что бы она двигалась
        x1 += dx
        y1 += dy
        ...
        #Проверяем съела ли наша змея еду и если да то даем ей новую координату
        ...
        if x1 == food1x and y1 == food1y:
            ...
            for point in wall.body:
                if food1x == snake_block * point.x and food1y == snake_block * point.y:
                    ...
                    continue
                tt = random.randint(2,3) 
            #За каждую съеденную еду либо 2 либо 3 очка
            snake_length += tt
